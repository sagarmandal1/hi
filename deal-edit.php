...
$dealItems = $dealModel->getItems($id);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        if ($result) {
            ...
            setFlashMessage('success', 'Deal updated successfully!');
            redirect('deal-view.php?id=' . $id);
        } else {
            $errors[] = 'Failed to update deal. Please try again.';
        }
    }
}
?>
...
<div class="page-header">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="deal-view.php?id=<?php echo $id; ?>"><?php echo sanitize($deal['deal_number']); ?></a></li>
            <li class="breadcrumb-item active">Edit</li>
        </ol>
    </nav>
    <h4>Edit Deal - <?php echo sanitize($deal['deal_number']); ?></h4>
</div>
...
<form method="POST" action="" id="dealForm">
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-md-3 mb-3">
                    ...
                    <input type="text" class="form-control" value="<?php echo sanitize($deal['deal_number']); ?>" readonly>
                </div>
                <div class="col-md-3 mb-3">
                    ...
                    <input type="date" class="form-control" id="deal_date" name="deal_date" 
                           value="<?php echo $_POST['deal_date'] ?? $deal['deal_date']; ?>" required>
                </div>
                <div class="col-md-3 mb-3">
                    ...
                    <select class="form-select" id="customer_id" name="customer_id" required>
                        ...
                        <?php foreach ($customers as $customer): ?>
                            <option value="<?php echo $customer['id']; ?>" 
                                    <?php echo (($_POST['customer_id'] ?? $deal['customer_id']) == $customer['id']) ? 'selected' : ''; ?>>
                ...
                <div class="col-md-3 mb-3">
                    ...
                    <select class="form-select" id="status" name="status">
                        <option value="pending" <?php echo (($_POST['status'] ?? $deal['status']) === 'pending') ? 'selected' : ''; ?>>Pending</option>
                        <option value="completed" <?php echo (($_POST['status'] ?? $deal['status']) === 'completed') ? 'selected' : ''; ?>>Completed</option>
                        <option value="cancelled" <?php echo (($_POST['status'] ?? $deal['status']) === 'cancelled') ? 'selected' : ''; ?>>Cancelled</option>
                    </select>
                </div>
            </div>
            <div class="mb-3">
                <label for="notes" class="form-label">Notes</label>
                <textarea class="form-control" id="notes" name="notes" rows="2"><?php echo sanitize($_POST['notes'] ?? $deal['notes']); ?></textarea>
            </div>
        </div>
    </div>
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            <div id="dealItemsContainer">
                <?php foreach ($dealItems as $index => $item): ?>
                <div class="deal-item-row">
                    <div class="row">
                        <div class="col-md-3 mb-2">
                            <label class="form-label">Product</label>
                            <select class="form-select product-select" name="items[<?php echo $index; ?>][product_id]" onchange="updateProductName(this)">
                        ...
                        <div class="col-md-3 mb-2">
                            ...
                            <input type="text" class="form-control product-name" name="items[<?php echo $index; ?>][product_name]" 
                                   value="<?php echo sanitize($item['product_name']); ?>" required>
                        </div>
                        <div class="col-md-2 mb-2">
                            <label class="form-label">Buy Qty</label>
                            <input type="number" step="0.01" min="0" class="form-control buy-qty" name="items[<?php echo $index; ?>][buy_qty]" 
                                   value="<?php echo $item['buy_quantity']; ?>" onchange="calculateItemTotal(this.closest('.deal-item-row'))">
                        </div>
                        <div class="col-md-2 mb-2">
                            <label class="form-label">Buy Price</label>
                            <input type="number" step="0.01" min="0" class="form-control buy-price" name="items[<?php echo $index; ?>][buy_price]" 
                                   value="<?php echo $item['buy_price']; ?>" onchange="calculateItemTotal(this.closest('.deal-item-row'))">
                        </div>
                        <div class="col-md-2 mb-2">
                            <label class="form-label">Total Buy</label>
                            <input type="number" step="0.01" class="form-control total-buy" name="items[<?php echo $index; ?>][total_buy]" 
                                   value="<?php echo $item['total_buy']; ?>" readonly>
                        </div>
                    </div>
                    <div class="row">
                        ...
                        <div class="col-md-2 mb-2">
                            ...
                            <input type="number" step="0.01" min="0" class="form-control sell-qty" name="items[<?php echo $index; ?>][sell_qty]" 
                                   value="<?php echo $item['sell_quantity']; ?>" onchange="calculateItemTotal(this.closest('.deal-item-row'))">
                        </div>
                        <div class="col-md-2 mb-2">
                            <label class="form-label">Sell Price</label>
                            <input type="number" step="0.01" min="0" class="form-control sell-price" name="items[<?php echo $index; ?>][sell_price]" 
                                   value="<?php echo $item['sell_price']; ?>" onchange="calculateItemTotal(this.closest('.deal-item-row'))">
                        </div>
                        <div class="col-md-2 mb-2">
                            <label class="form-label">Total Sell</label>
                            <input type="number" step="0.01" class="form-control total-sell" name="items[<?php echo $index; ?>][total_sell]" 
                                   value="<?php echo $item['total_sell']; ?>" readonly>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-10 mb-2">
                            <label class="form-label">Item Profit</label>
                            <input type="number" step="0.01" class="form-control item-profit" name="items[<?php echo $index; ?>][profit]" 
                                   value="<?php echo $item['profit']; ?>" readonly>
                        </div>
        ...
        <div class="card-body">
            <div class="row text-center">
                <div class="col-md-4">
                    ...
                    <h3 class="text-info" id="grandTotalBuy"><?php echo formatCurrency($deal['total_buy_amount']); ?></h3>
                </div>
                <div class="col-md-4">
                    <h5>Total Sell Amount</h5>
                    <h3 class="text-primary" id="grandTotalSell"><?php echo formatCurrency($deal['total_sell_amount']); ?></h3>
                </div>
                <div class="col-md-4">
                    <h5>Profit</h5>
                    <h3 class="<?php echo $deal['profit'] >= 0 ? 'text-success' : 'text-danger'; ?>" id="grandProfit">
    ...
    <div class="d-flex gap-2 mb-4">
        <button type="submit" class="btn btn-primary btn-lg">
            <i class="bi bi-check-circle me-2"></i>Update Deal
        </button>
        <a href="deal-view.php?id=<?php echo $id; ?>" class="btn btn-outline-secondary btn-lg">Cancel</a>
    </div>
</form>
...
$extraScripts = <<<'SCRIPT'
<script>
let itemCounter = document.querySelectorAll('.deal-item-row').length;
